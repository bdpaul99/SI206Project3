...
def get_user_tweets(twitter_handle):
	if twitter_handle in CACHE_DICTION:
		...
		return api.user_timeline(screen_name = twitter_handle)
...
for tweet in umsi_tweets:
	tweet_touple = (tweet['id'], tweet['user']['screen_name'], tweet['created_at'], tweet['text'], tweet['retweet_count'])
...
def get_twitter_users(in_string):
	user_names = (re.findall(r'@([A-Za-z0-9_]+)',in_string))
